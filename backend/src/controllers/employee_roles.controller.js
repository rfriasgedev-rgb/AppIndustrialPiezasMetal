...
exports.getAll = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM employee_roles ORDER BY name');
        res.json(rows);
    } catch (error) {
...
exports.getById = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM employee_roles WHERE id = ?', [req.params.id]);
...
exports.update = async (req, res) => {
    try {
        const { name, description } = req.body;
        const [result] = await pool.query('UPDATE employee_roles SET name = ?, description = ? WHERE id = ?', [name, description, req.params.id]);

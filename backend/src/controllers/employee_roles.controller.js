...
exports.getAll = async (req, res) => {
    try {
        const [rows] = await db.pool.query('SELECT * FROM employee_roles ORDER BY name');
        res.json(rows);
    } catch (error) {
...
exports.getById = async (req, res) => {
    try {
        const [rows] = await db.pool.query('SELECT * FROM employee_roles WHERE id = ?', [req.params.id]);
...
exports.create = async (req, res) => {
    try {
        ...
        const id = uuidv4();
        await db.pool.query('INSERT INTO employee_roles (id, name, description) VALUES (?, ?, ?)', [id, name, description]);
...
exports.update = async (req, res) => {
    try {
        const { name, description } = req.body;
        const [result] = await db.pool.query('UPDATE employee_roles SET name = ?, description = ? WHERE id = ?', [name, description, req.params.id]);
...
exports.delete = async (req, res) => {
    try {
        const [result] = await db.pool.query('DELETE FROM employee_roles WHERE id = ?', [req.params.id]);
        if (result.affectedRows === 0) return res.status(404).json({ error: 'Role not found' });
